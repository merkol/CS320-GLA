...
public class Database {
	...
	public static void main(String[] args) {
		try {
			...
			String createTableQueryUsers = "CREATE TABLE IF NOT EXISTS Users ("
					...
					+ "    password VARCHAR(20), balance FLOAT(5)  CHECK (balance >= 0));";
			...
			while (true) {
				...
				case "8": {
					...
					statement.executeUpdate("ALTER TABLE Games AUTO_INCREMENT = 0;");
					statement.executeUpdate("DELETE FROM Game_Ownership WHERE uid>0 AND gid>0;");
					statement.executeUpdate("ALTER TABLE Game_Ownership AUTO_INCREMENT = 0;");
					break;
				}
	...
	public static ArrayList<String> listGameInfo(int gid) {
		...
		try {
			...
			ResultSet rs = stmt.executeQuery();
			while (rs.next()) {
				info.add(rs.getString("Games.name"));
				info.add(rs.getString("Games.description"));
				info.add(rs.getString("Games.publisher"));
				info.add("" + rs.getFloat("Games.price"));
				info.add("" + rs.getFloat("Games.score"));
				info.add("" + rs.getBoolean("Games.is_multi"));
			}
			rs.close();
		} catch (SQLException ex) {
			ex.printStackTrace();
